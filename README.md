...
This repository contain number of services to discover and manage some hardware
on Yadro VEGMAN BMC platform.

## yadro-hw-manager

This is generic detection daemon that helps EntityManager to detect entities.
Currently yadro-hw-manager provide information about chassis and fans:
...
Each node exposes corresponding dbus interface: `com.yadro.HWManager.Chassis`
or `com.yadro.HWManager.Fan` Interface definition can be found in `com`
directory.

## yadro-storage-manager

Storage manager aimed to provide inventory information about storage devices
and manage their state. It also communicates with backplane to manage some
functions:
...
Storage manager sits in `/com/yadro/storage` and currently only have function
to locate storage drive in backplane:

```
~# busctl introspect com.yadro.Storage /com/yadro/storage
NAME                                TYPE      SIGNATURE RESULT/VALUE FLAGS
com.yadro.HWManager.StorageManager  interface -         -            -
.SetDriveLocationLED                method    sb        b            -
...
* `Rescan` method in `com.yadro.Inventory.Manager` trigger reread drives list,
  received from BIOS.
