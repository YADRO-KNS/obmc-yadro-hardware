...
const FanModuleInfo& getFanInfoByRPM(
    const std::vector<FanModuleInfo>& detectionFanTable,
    ...
    io(io), bus(bus), powerState(bus)
{
    ...
    powerState.addCallback(
        "manager",
        std::bind(&HWManager::onHostPowerChanged, this, std::placeholders::_1));
}
    ...
    for (const auto& [conIndex, conDescr] : config.desc->fans)
    {
        ...
        if (conDescr.type == ConnectorType::SYSTEM)
        {
            ...
            fans.emplace_back(std::make_shared<Fan>(
                bus, sysFanPrefixName + fanIndexStr, prettyName,
