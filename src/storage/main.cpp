...
class Manager : InventoryManagerServer, StorageManagerServer
{
  public:
    ...
    void rescan();
    bool setDriveLocationLED(std::string driveSN, bool assert);
...
bool Manager::setDriveLocationLED(std::string driveSN, bool assert)
{
    for (const auto& [chan, mcu] : bplMCUs)
    {
        if (mcu->setDriveLocationLED(driveSN, assert))
        {
            return true;
        }
    }
