...
int i2cDev::read_i2c_block_data(uint8_t command, uint8_t length,
                                uint8_t* values)
{
    int res = i2c_smbus_read_i2c_block_data(devFD, command, length, values);
...
int i2cDev::read_i2c_blob(uint8_t command, uint8_t length, uint8_t* values)
{
    ...
    int res = ioctl(devFD, I2C_RDWR, &i2c_req);
    logTransfer(command, nullptr, 0, values, length, res);
    return res;
}

void i2cDev::logTransfer(uint8_t cmd, const void* txData, size_t txDataLen,
